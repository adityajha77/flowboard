// Simple Kanban app with localStorage + drag/drop + PRIORITY
const COLUMNS = ['Backlog','To Do','In Progress','Done'];
const STORAGE_KEY = 'flowboard-priority:v1';
...
function load(){
  ...
  if(raw) state = JSON.parse(raw);
    ...
    // sample card
    state.columns['Backlog'].push({
      id:uid(), 
      title:'Welcome to FlowBoard', 
      desc:'Drag me to different columns', 
      assignee:'Team',
      priority:'medium'
    });
    save();
...
function render(){
  ...
  COLUMNS.forEach(colName => {
    ...
    list.addEventListener('drop', e => {
      ...
      const originalCard = getCardFromElement(cardEl);
      if(originalCard) {
        state.columns[col].splice(index, 0, originalCard);
        save(); 
        render();
      }
    });
...
function getCardFromElement(el){
  ...
  // find the card object from state (preserves priority)
...
saveCardBtn.addEventListener('click', () => {
  ...
  const card = { 
    id: editing ? state.columns[editing.col][editing.idx].id : uid(), 
    title: t, 
